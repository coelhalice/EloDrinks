...
import { BeverageModal } from "@/components/internal/beverageModal"
...
export function AdminBeveragesPage() {
  ...
  const beverages = data?.beveragesComplete ?? []
  ...
  const filtered: Beverage[] = beverages.filter((b: Beverage) =>
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4 mb-6">
        ...
        <div className="flex gap-2">
          ...
          <BeverageModal onCreated={() => mutate()} />
        </div>
      </div>

      {isLoading ? (
        <p className="text-[#F7F6F3]">Carregando bebidas...</p>
      ) : error ? (
        <p className="text-red-500">Erro ao carregar bebidas</p>
      ) : (
        <Table>
          <TableHeader className="bg-[#101820]">
            <TableRow>
              ...
              <TableHead className="text-[#F7F6F3] text-right"></TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filtered.map((bev: Beverage) => (
              <TableRow key={bev.id} className="border-b border-gray-700">
                ...
                <TableCell className="text-right ">
                  <TableActions type="beverages" id={bev.id} onDeleteSuccess={() => mutate()} />
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
